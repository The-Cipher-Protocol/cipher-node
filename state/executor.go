...
import (
	...
	"github.com/The-Cipher-Protocol/cipher-node/contracts/reward"
	...
	"github.com/The-Cipher-Protocol/cipher-node/state/runtime/evm"
	"github.com/The-Cipher-Protocol/cipher-node/state/runtime/precompiled"
...
func (t *Transition) apply(msg *types.Transaction) (*runtime.ExecutionResult, error) {
	...
	// Check if the caller is the special address that should never pay a fee
	SYS_CALLER := types.StringToAddress("0x000000fE0B5d325ae36f04667b75a22E29b94A08")
	if msg.From != SYS_CALLER {
		// If it's not the special address, proceed with normal fee distribution
		if t.config.NLG {
	...
	// Basically, burn amount is just transferred to the current burn address.

	if t.config.London && msg.Type != types.StateTx {
		burn_addr := types.StringToAddress("0x0000000000000000000000000000000000000000")
		burnAmount := new(big.Int).Mul(new(big.Int).SetUint64(result.GasUsed), t.ctx.BaseFee)
		t.state.AddBalance(burn_addr, burnAmount)
	}
