...
contract Staking {
    ...
    uint256 public constant EPOCH_BLOCKS = 200;
    ...
    // Pending validators
    mapping(address => uint256) public _pendingValidatorStartBlock;
    address[] public _pendingValidators;

    // Events
    event Staked(address indexed account, uint256 amount);
    event Unstaked(address indexed account, uint256 amount);
    event BLSPublicKeyRegistered(address indexed accout, bytes key);
    event MovedToPendingState(address indexed account);
    event AddedToValidatorSet(address indexed account);
    ...
    function checkPendingValidators() public {
        _checkPendingValidators();
    }
    ...
    function _stake() private {
        ...
        if (_canMoveToPendingState(msg.sender)) {
            _moveToPendingState(msg.sender);
        }
    ...
    function _moveToPendingState(address staker) private {
        _pendingValidators.push(staker);
        _pendingValidatorStartBlock[staker] = block.number;

        emit MovedToPendingState(staker);
    }

    function _checkPendingValidators() private {
        uint256 currentBlock = block.number;

        for (uint256 i = 0; i < _pendingValidators.length; i++) {
            address pendingValidator = _pendingValidators[i];
            uint256 startBlock = _pendingValidatorStartBlock[pendingValidator];

            if (currentBlock >= startBlock + (2 * EPOCH_BLOCKS)) {
                _appendToValidatorSet(pendingValidator);

                // Remove from pending state
                _pendingValidators[i] = _pendingValidators[_pendingValidators.length - 1];
                _pendingValidators.pop();
                _pendingValidatorStartBlock[pendingValidator] = 0;

                emit AddedToValidatorSet(pendingValidator);
            }
        }
    }
    ...
    function _canMoveToPendingState(address account) private view returns (bool) {
        return
            !_isValidator(account) &&
            _addressToStakedAmount[account] >= VALIDATOR_THRESHOLD &&
            _pendingValidatorStartBlock[account] == 0;
    }
}