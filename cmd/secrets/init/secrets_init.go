...
func GetCommand() *cobra.Command {
	secretsInitCmd := &cobra.Command{
		Use: "init",
		Short: "Initializes private keys for the NETH (Validator + Networking) " +
...
func setFlags(cmd *cobra.Command) {
	cmd.Flags().StringVar(
		...
		dataDirFlag,
		"",
		"the directory for the NETH data if the local FS is used",
	)
	...
	// Don't accept datadir and config flags because they are related to different secrets managers.
	// datadir is about the local FS as secrets storage, config is about remote secrets manager.
	cmd.MarkFlagsMutuallyExclusive(dataDirFlag, configFlag)

	// num flag should be used with datadir flag only so it should not be used with config flag.
